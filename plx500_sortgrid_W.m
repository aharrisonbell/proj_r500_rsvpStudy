...
if ismember(1,option)==1,
    ...
    for g=1:numgrids
        ...
        if g==1, figure_title(get(gca),'Traditional Face Selectivity',12); end
    end
    ...
    for g=1:numgrids
        ...
        if g==1, figure_title(get(gca),'Proportion of face-preferring neurons that are face selective (trad)',12); end
    end
...
if ismember(3,option)==1,
    for g=1:numgrids,
        ...
        mkdir([hmiconfig.rsvpanal,'GridBreakDown_W',filesep,char(grids(g))]);
        ...
        for un=1:length(gridind),
           % try 
                copyfile(...
                    ...
                    [hmiconfig.rsvpanal,'GridBreakDown_W',filesep,char(grids(g)),filesep,char(allunits(gridind(un)).NewUnitName),'_rsvp500_Mean Spden.jpg']);
...
%%%% OLD FIGURES
% figure; clf; cla; % selectivity index (average)
% set(gcf,'Units','Normalized','Position',[0.05 0.05 0.9 0.9])
% set(gca,'FontName','Arial','FontSize',8)
% for g=1:numgrids
%     subplot(5,5,g)
%     hold on
%     bar(mean(data(g).cat_si))
%     errorbar(1:6,mean(data(g).cat_si),sem(data(g).cat_si))
%     title([char(grids(g)),' (n=',num2str(data(g).numunits),')'],'FontSize',fontsize_lrg)
%     set(gca,'XTick',1:6,'XTickLabel',{'F','Ft','P','Bp','O','FnBp'},'FontSize',fontsize_sml); xlim([0.5 6.5])
%     ylim([-.3 .3])
%     if g==1, figure_title(get(gca),'Selectivity Index (all) uncorrected',12); end
% end
%
% figure; clf; cla; % selectivity index (average) nobaseline
% set(gcf,'Units','Normalized','Position',[0.05 0.05 0.9 0.9])
% set(gca,'FontName','Arial','FontSize',8)
% for g=1:numgrids
%     subplot(5,5,g)
%     hold on
%     bar(mean(data(g).cat_si_nobase))
%     errorbar(1:6,mean(data(g).cat_si_nobase),sem(data(g).cat_si_nobase))
%     title([char(grids(g)),' (n=',num2str(data(g).numunits),')'],'FontSize',fontsize_lrg)
%     set(gca,'XTick',1:6,'XTickLabel',{'F','Ft','P','Bp','O','FnBp'},'FontSize',fontsize_sml); xlim([0.5 6.5])
%     %ylim([-1 1])
%     if g==1, figure_title(get(gca),'Selectivity Index (all, nobaseline) uncorrected',12); end
% end
% figure; clf; cla; % selectivity index (average) corrected
% set(gcf,'Units','Normalized','Position',[0.05 0.05 0.9 0.9])
% set(gca,'FontName','Arial','FontSize',8)
% for g=1:numgrids
%     subplot(5,5,g)
%     hold on
%     [bardata bardata_sem]=mean_noncol(data(g).cat_si(:,1:5) .* data(g).countsmat);
%     bar(bardata)
%     errorbar(1:5,bardata,bardata_sem)
%     title([char(grids(g)),' (n=',num2str(data(g).numunits),')'],'FontSize',fontsize_lrg)
%     set(gca,'XTick',1:5,'XTickLabel',{'F','Ft','P','Bp','O'},'FontSize',fontsize_sml); xlim([0.5 5.5])
%     ylim([0 1])
%     if g==1, figure_title(get(gca),'Selectivity Index (all) Corrected',12); end
% end
%
% figure; clf; cla; % selectivity index (average) no baseline corrected
% set(gcf,'Units','Normalized','Position',[0.05 0.05 0.9 0.9])
% set(gca,'FontName','Arial','FontSize',8)
% for g=1:numgrids
%     subplot(5,5,g)
%     hold on
%     [bardata bardata_sem]=mean_noncol(data(g).cat_si_nobase(:,1:5) .* data(g).countsmat);
%     bar(bardata)
%     errorbar(1:5,bardata,bardata_sem)
%     title([char(grids(g)),' (n=',num2str(data(g).numunits),')'],'FontSize',fontsize_lrg)
%     set(gca,'XTick',1:5,'XTickLabel',{'F','Ft','P','Bp','O'},'FontSize',fontsize_sml); xlim([0.5 5.5])
%     if g==1, figure_title(get(gca),'Selectivity Index (all, nobaseline) Corrected',12); end
% end
