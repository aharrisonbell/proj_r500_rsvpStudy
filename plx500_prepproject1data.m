...
for un=1:size(allunits,2),
    ...
    unitdata.baseline(un)=mean(respstructsingle.m_baseline);
    unitdata.cat_avg(un,:)=respstructsingle.cat_avg(:,2)';
    unitdata.roc_analysis(un,:)=reshape(respstructsingle.roc_analysis',1,25);
    unitdata.latency(un,:)=respstructsingle.latency([1:20 61:80 81:100 41:60])';
...
save([hmiconfig.rsvpanal,filesep,sheetname(12),'_data.mat'],'data','numgrids','counts_matrix','allunits','unit_index','unitdata');
